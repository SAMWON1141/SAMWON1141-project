...
export default async function HomePage() {
  ...
  const LogoDisplay = ({
    ...
    > = {
      md: { container: "h-10 w-10", icon: "h-6 w-6", text: "text-base" },
      xl: { container: "h-16 w-16", icon: "h-10 w-10", text: "text-xl" },
    };

    const sizeConfig = sizeMap[size] || sizeMap.md;

    if (displaySettings.logo) {
      return (
        <div className="flex items-center gap-2">
          <div
            className={`relative overflow-hidden rounded-lg ${sizeConfig.container}`}
          >
            <Image
              src={`/uploads/${displaySettings.logo}?t=${Date.now()}`}
    ...
    return (
      <div className="flex items-center gap-2">
        <div
          className={`flex items-center justify-center rounded-lg bg-primary ${sizeConfig.container}`}
        >
